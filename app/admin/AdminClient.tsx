...
export default function AdminClient({user,signOutPath}:{user:ChatGPTUser;signOutPath:string}){
 ...
 const selectMedia=(file?:File)=>{if(!file)return;setMediaFile(file);setForm(x=>({...x,imageKey:null,imageUrl:URL.createObjectURL(file)}));setStatus(`"${file.name}" 선택됨 · 저장하기를 눌러 등록하세요.`)};
 const upload=async(file:File)=>{const data=new FormData();data.append("file",file);const r=await fetch("/api/upload",{method:"POST",body:data}),d=await r.json();if(!r.ok)throw new Error(d.error);return d as {key:string;url:string}};
 ...
 return <main className="admin-shell"><header className="admin-header"><Link href="/">SIMSIMENT®</Link><div><span>{user.displayName}</span><a href={signOutPath}>로그아웃</a></div></header><section className="admin-intro"><p>CONTENT CONTROL</p><h1>사이트 관리</h1><span>{status}</span></section><div className="admin-layout">
  <form className="editor" onSubmit={submit}><div className="editor-title"><h2>{form.id?"콘텐츠 수정":"새 콘텐츠"}</h2>{form.id&&<button type="button" onClick={reset}>신규 작성</button>}</div>
   ...
   <label className="upload-box">사진 / 영상 / 앨범 아트<input type="file" accept="image/jpeg,image/png,image/webp,image/gif,video/mp4,video/webm,video/quicktime" onChange={e=>selectMedia(e.target.files?.[0])}/><span>{mediaFile?`${mediaFile.name} · 저장 시 업로드`:form.imageUrl?"미디어 변경":"사진 12MB · 영상 100MB 이하"}</span>{form.imageUrl&&((mediaFile?.type.startsWith("video/")||/\.(mp4|webm|mov)$/i.test(form.imageKey??""))?<video src={form.imageUrl} controls muted playsInline/>:<img src={form.imageUrl} alt="업로드 미리보기"/>)}</label>
