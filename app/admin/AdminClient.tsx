...
export default function AdminClient({user,signOutPath}:{user:ChatGPTUser;signOutPath:string}){
 ...
 const upload=async(file?:File)=>{if(!file)return;setBusy(true);setStatus("이미지 업로드 중...");const data=new FormData();data.append("file",file);try{const r=await fetch("/api/upload",{method:"POST",body:data}),d=await r.json();if(!r.ok)throw new Error(d.error);setForm(x=>({...x,imageKey:d.key,imageUrl:d.url}));setStatus("이미지가 업로드되었습니다.")}catch(e){setStatus(e instanceof Error?e.message:"업로드 실패")}finally{setBusy(false)}};
 ...
 return <main className="admin-shell"><header className="admin-header"><Link href="/">SIMSIMENT®</Link><div><span>{user.displayName}</span><a href={signOutPath}>로그아웃</a></div></header><section className="admin-intro"><p>CONTENT CONTROL</p><h1>사이트 관리</h1><span>{status}</span></section><div className="admin-layout">
  <form className="editor" onSubmit={submit}><div className="editor-title"><h2>{form.id?"콘텐츠 수정":"새 콘텐츠"}</h2>{form.id&&<button type="button" onClick={()=>setForm(blank)}>신규 작성</button>}</div>
   ...
   <label className="upload-box">사진 / 앨범 아트<input type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={e=>upload(e.target.files?.[0])}/><span>{form.imageUrl?"이미지 변경":"이미지 선택 (최대 12MB)"}</span>{form.imageUrl&&<img src={form.imageUrl} alt="업로드 미리보기"/>}</label>
