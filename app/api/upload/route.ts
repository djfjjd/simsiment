...
export async function POST(request:Request){
  ...
  const max=file.type.startsWith("video/")?100*1024*1024:12*1024*1024;
  if(file.size>max) return Response.json({error:file.type.startsWith("video/")?"영상은 100MB 이하여야 합니다.":"이미지는 12MB 이하여야 합니다."},{status:400});
